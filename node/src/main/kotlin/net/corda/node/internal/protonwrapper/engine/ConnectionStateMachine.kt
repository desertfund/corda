...
class ConnectionStateMachine(serverMode: Boolean,
    ...
    override fun onConnectionInit(event: Event) {
        ...
        log.info("Connection init $connection")
    }
    ...
    override fun onConnectionFinal(event: Event) {
        val connection = event.connection
        log.info("Connection final $connection")
    ...
    override fun onTransportHeadClosed(event: Event) {
        ...
        log.info("Transport Head Closed $transport")
        transport.close_tail()
    }

    override fun onTransportTailClosed(event: Event) {
        val transport = event.transport
        log.info("Transport Tail Closed $transport")
    ...
    override fun onSessionInit(event: Event) {
        ...
        log.info("Session init $session")
    }

    override fun onSessionLocalOpen(event: Event) {
        val session = event.session
        log.info("Session local open $session")
    }
    ...
    override fun onSessionLocalClose(event: Event) {
        val session = event.session
        log.info("Session local close $session")
        session.close()
        session.free()
    }
    ...
    override fun onSessionFinal(event: Event) {
        val session = event.session
        log.info("Session final $session")
    ...
    override fun onLinkLocalOpen(event: Event) {
        val link = event.link
        if (link is Sender) {
            log.info("Sender Link local open ${link.name} ${link.source} ${link.target}")
    ...
    override fun onLinkRemoteOpen(event: Event) {
        ...
        if (link is Receiver) {
            if (link.remoteTarget is Coordinator) {
                log.info("Coordinator link received")
            }
        }
    }
    ...
    override fun onLinkFinal(event: Event) {
        val link = event.link
        if (link is Sender) {
            log.info("Sender Link final ${link.name} ${link.source} ${link.target}")
    ...
    override fun onLinkFlow(event: Event) {
        val link = event.link
        if (link is Sender) {
            log.info("Sender Flow event: ${link.name} ${link.source} ${link.target}")
            if (senders.containsKey(link.target.address)) {
                transmitMessages(link)
            }
        } else if (link is Receiver) {
            log.info("Receiver Flow event: ${link.name} ${link.source} ${link.target}")
        }
    }
    ...
    private fun transmitMessages(sender: Sender) {
        ...
        while (sender.credit > 0) {
            log.info("Sender credit: ${sender.credit}")
            ...
            if (nextMessage != null) {
                try {
                    ...
                    log.info("Put tag ${javax.xml.bind.DatatypeConverter.printHexBinary(delivery.tag)} on wire uuid: ${nextMessage.applicationProperties["_AMQ_DUPL_ID"]}")
    ...
    override fun onDelivery(event: Event) {
        ...
        if (link is Receiver) {
            if (delivery.isReadable && !delivery.isPartial) {
                ...
                if (channel != null) {
                    ...
                    val receivedMessage = ReceivedMessageImpl(
                            ...
                            channel,
                            delivery)
                    log.info("Full message received uuid: ${appProperties["_AMQ_DUPL_ID"]}")
            ...
            log.info("Sender delivery confirmed tag ${javax.xml.bind.DatatypeConverter.printHexBinary(delivery.tag)}")
    ...
    fun transportWriteMessage(msg: SendableMessageImpl) {
        log.info("Queue application message write uuid: ${msg.applicationProperties["_AMQ_DUPL_ID"]} ${javax.xml.bind.DatatypeConverter.printHexBinary(msg.payload)}")
